...
function isPalindrome(str) {
  ...
  const isPal =
    revStr.toLowerCase().replace(/[^a-z0-9]/g, '') ===
...
function encodeToRot13(str) {
  const strUpp = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  const strLow = 'abcdefghijklmnopqrstuvwxyz';
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    if (!strUpp.includes(str.charAt(i)) && !strLow.includes(str.charAt(i))) {
      result += str.charAt(i);
    } else {
      const index = strUpp.indexOf(str.charAt(i).toLocaleUpperCase());
      const newIndex = index < 13 ? index + 13 : index - 13;
      if (str.charAt(i) === str.charAt(i).toUpperCase()) {
        result += strUpp.charAt(newIndex);
      } else {
        result += strLow.charAt(newIndex);
      }
    }
  }
